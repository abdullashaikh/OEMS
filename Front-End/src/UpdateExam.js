...
function CreateExam() {
  ...
  const [{ updateExamId }, dispatch] = useDataLayerValue();
  ...
  useEffect(() => {
    ...
    Axios.get(`http://localhost:3300/exam?id=${updateExamId}`).then((data) => {
      if (data.status === 200) {
        // console.log(data.data.body.data.standardId);
        setExamName(data.data.body.data.examName);
        setSelectedDate(data.data.body.data.examDateAndTime);
        setExamTotalScore(data.data.body.data.examTotalScore);
        setExamPassingScore(data.data.body.data.examPassingScore);
        // console.log("Standard", test_data);
        test_data.map((item) => {
          if (item._id == data.data.body.data.standardId) {
            console.log(item._id);
            console.log("Standard Id", data.data.body.data.standardId);
            setSelectStandard(item.name);
          }
        });
      }
    });
  ...
  const submit = () => {
    ...
    if (selectTeacher) {
      Axios.put("http://localhost:3300/exam", {
        ...
        if (data.status == 200) {
          console.log(data.data.body.data._id);
          dispatch({ type: "SET_EXAM_ID", examId: data.data.body.data._id });
      ...
      alert("First You need to login to create Exam");
    }
  };
